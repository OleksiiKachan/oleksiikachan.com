...
*/
class BackToTopButton extends Component<any, any> {
  offset: number;
  ...
  setButtonConfigurations = () => {
    if (this.backTop) {
      ...
      this.backTop.addEventListener('click', (event: any) => {
  ...
  checkBackToTop = () => {
    ...
    windowTop > this.offset
      ? this.addClass(this.backTop, 'backToTopButton--show')
      : this.removeClass(
          this.backTop,
          classNames('backToTopButton--show', 'backToTopButton--fade-out')
        );
    windowTop > this.offsetOpacity &&
      this.addClass(this.backTop, 'backToTopButton--fade-out');
    this.scrolling = false;
  };

  scrollTop = (duration: number) => {
    var start = window.scrollY || document.documentElement!.scrollTop,
      currentTime: any = null;

    var animateScroll = (timestamp: any) => {
      if (!currentTime) currentTime = timestamp;
      var progress = timestamp - currentTime;
      var val = Math.max(
        this.easeInOutQuad(progress, start, -start, duration),
        0
      );
  ...
  render() {
    return (
      <a
        href={this.props.topId}
        className={classNames('backToTopButton', 'backToTopButton_js')}
      >
        Top
      </a>
    );
