...
*/
class LanguagesPanel extends Component<any, any> {
  createLanguageRate = (rate: number) => {
    let visualRate = [];
    rate = rate > 5 ? 5 : rate;
    ...
    for (let i = 0; i < rate; i++) {
      visualRate.push(
        <img
          src={FilledStar}
          alt="Filled blue star."
          className={classNames('languageItem__circleRate')}
        />
      );
    }
    ...
    return (
      <ul className={classNames(this.props.className)}>
        {this.props.languages.map(
          (language: { language: string; stars: number }) => {
            return (
              <li className={classNames('languageItem')}>
                <p className={classNames('languageItem__name')}>
                  {language.language}
                </p>
